...
light_DI = sys.argv[3] # enable light Demographic inference mode ; define if the number of loci simulation is limited to 1000 or not ; True = limit of 1000 loci , False = no limit
...
nLoci = len(L)
if light_DI == True:
    # if light_DI is True only a subsample of 1000 loci or less (if nLoci < 1000) 
    threshold_sim = 1000
else :
    # if light_DI is false, all loci will be used 
    threshold_sim = nLoci
...
if nLoci < threshold_sim:
	...
	# if more than threshold_sim, then subsample 1000 or all loci (depending of the value of light_DI) for simulations only
	nLoci_sim = threshold_sim
	sampled_loci = sample(range(nLoci), nLoci)
